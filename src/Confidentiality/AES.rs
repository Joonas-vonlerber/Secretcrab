...
/// Enum for specifying the key size used in the AES algorithm
pub enum AESKeySize {
    Key128,
    Key192,
    Key256,
...
fn GF_mul(lhs: u8, rhs: u8) -> u8 {
    match lhs {
        2 => GF_MUL_2[rhs as usize],
        3 => GF_MUL_3[rhs as usize],
        9 => GF_MUL_9[rhs as usize],
        0x0b => GF_MUL_B[rhs as usize],
        0x0d => GF_MUL_D[rhs as usize],
        0x0e => GF_MUL_E[rhs as usize],
        _ => unimplemented!(),
    }
}
...
pub fn generate_aes_key(key_size: &AESKeySize) -> Vec<u32> {
    ...
    output.truncate(key_size.key_size_in_32_words());
    output
}

/// Encryption using the AES algorithm with the given key.
///
/// ## Keys
/// Keys can be generated by the [generate_aes_key] function or with your own algorithm.
///
/// If you want to generate your own keys, you need them to be slice of 4 byte words and it must be the same size in bits as the key_size argument
///
/// ## Panics
///
/// If the key is not the same size as is specified in the key_size argument. If using [generate_aes_key] this should not be a problem.
pub fn aes_encryption(input: &[u8], key: &[u32], key_size: &AESKeySize) -> Vec<u8> {
    assert_eq!(key.len(), key_size.key_size_in_32_words());
    let mut blocks: Vec<AESState> = bytes_to_block(&pad_PKCS5_message(input));
    blocks
        .iter_mut()
        .for_each(|block: &mut AESState| encryption(block, key, key_size));
    blocks
        .into_iter()
        .flat_map(|block| block.into_raw_vec())
        .collect::<Vec<_>>()
}

/// Decryption using the AES algorithm which was encrypted with the given key.
///
/// ## Keys
/// [read more](aes_encryption) on keys
///
/// ## Panics
/// If the key is not the same size as is specified in the key_size argument. If using [generate_aes_key] this should not be a problem.
pub fn aes_decryption(input: &[u8], key: &[u32], key_size: &AESKeySize) -> Vec<u8> {
    assert_eq!(key.len(), key_size.key_size_in_32_words());
    let mut blocks: Vec<AESState> = bytes_to_block(input);
    blocks
        .iter_mut()
        .for_each(|block: &mut AESState| decryption(block, key, key_size));
    let output = blocks
        .into_iter()
        .flat_map(|block| block.into_raw_vec())
        .collect::<Vec<_>>();
    unpad_PKCS5_message(&output)
}
...
fn padding_test() {
    ...
    let padded_message = pad_PKCS5_message(&message);
    assert_eq!(
        padded_message,
        vec![
    ...
    let depadded_message = unpad_PKCS5_message(&padded_message);
    assert_eq!(message.to_vec(), depadded_message);
}

#[test]
fn public_interface_encryption_decryption_test() {
    let message = b"Yaas queen SLAYYYYYY";
    let keysize: AESKeySize = AESKeySize::Key256;
    let key = generate_aes_key(&keysize);
    let encrypted_message = aes_encryption(message, &key, &keysize);
    let decrypted_message = aes_decryption(&encrypted_message, &key, &keysize);
